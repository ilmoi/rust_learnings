...
/*
todo: learnings
 - main always runs on ThreadId 1 (expected)
 - measure_task_q, because it's synchronous also always runs on ThreadId 1 (expected)
 - async tasks spawn on random threads - ThreadId 4, 5, 10, whatever
 - dropping Mutex quickly (via a block) is important for 1)calling sleep.await, 2)letting others grab it
 - note that tokio::spawn takes an async block {}, NOT an async closure || {}
 - in this case I made the entire execute_task() function async - but it's possible to also make just the redis call async. I don't see the point though, because the call is lightning quick while the processing is the slow part, so I didn't bother
 */
...
pub async fn main() {
    ...
    loop {
        ...
        let high = measure_task_q(arc_conn.clone(), "high");
        measure_task_q(arc_conn.clone(), "low"); //just for printing purposes
...
pub fn measure_task_q(conn_clone: Arc<Mutex<Connection>>, q_name: &str) -> isize {
